...
TEXT1 		:= Dolphin Reader
TEXT2 		:= by Chris
TEXT3 		:= http://www.chrislg.com
ICON 		:= -b /c/DR_Logo.bmp
LOGO		:= -o /c/logo_wifi.bmp
...
CFLAGS	+=	-I../../include -I../../fat -DNDS -I../../ConvertUTF -DUSE_DOUBLE_FRAME_BUFFER -DFAT

ASFLAGS	:=	-g $(ARCH)
LDFLAGS	:=	-g $(ARCH) -mno-fpu -L$(DEVKITPRO)/PAlib/lib
LDFLAGS	+=	-Wl,-Map,dr.map
...
#---------------------------------------------------------------------------------
LIBS	:= -lfat -lnds9 -ldswifi9 -lnds7 -lutf
LIBSPA	:= -lpa9
...
$(BUILD):
	@[ -d $@ ] || mkdir -p $@
	@make --no-print-directory -C $(BUILD) -f $(CURDIR)/dr.mak
...
%.ds.gba: %.nds
	...
	@cp $(CURDIR)/../$(notdir $@) $(CURDIR)/../$(TARGET)_tmp.ds.gba
	@padbin 512 $(CURDIR)/../$(TARGET)_tmp.ds.gba
	@cat $(CURDIR)/../$(TARGET)_tmp.ds.gba $(CURDIR)/../disk.img > $(CURDIR)/../$(TARGET)_fs.ds.gba
	@dlditool $(CURDIR)/../fcsr.dldi $(CURDIR)/../$(TARGET)_fs.ds.gba
